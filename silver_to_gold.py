...
def process_silver_to_gold():
    ...
    spark = SparkSession.builder.master("local[*]").appName("SilverToGold").getOrCreate()

    # Load tables from the silver layer
    athlete_bio_df = spark.read.parquet("/tmp/data_sharing/silver/athlete_bio")
    athlete_event_results_df = spark.read.parquet("/tmp/data_sharing/silver/athlete_event_results")
    ...
    output_path = "/tmp/data_sharing/gold/avg_stats"
