...
def process_table(table):
    ...
    spark = SparkSession.builder.master("local[*]").appName("BronzeToSilver").getOrCreate()

    # Read the table from the bronze layer
    input_path = f"/tmp/data_sharing//bronze/{table}"
    ...
    output_path = f"/tmp/data_sharing//silver/{table}"
